...
class DzForumPost extends DiscuzAR {
    ...
    //获取附件信息
    public static function getTableid($tid) {
        return DbUtils::getDzDbUtils(true)->queryRow('
            SELECT tableid
            FROM %t
            WHERE tid=%d
        ',
            array('forum_attachment ', $tid)
        );
    }
    public static function getFujian($tid, $tableid) {
    return DbUtils::getDzDbUtils(true)->queryAll('
        ...
        ',
        array('forum_attachment_'.$tableid, $tid)
    );
}
    ...
    public static function transPostContentToHtml($post) {
        ...
        if(($post['groupid']==4||$post['groupid']=='5')&&!$_G['forum']['ismoderator']){
            $postlist[$pid]['message'] = WebUtils::t('该用户被管理员或版主屏蔽');
        }
        // 处理屏蔽
        if (!$_G['forum']['ismoderator'] && getstatus($postlist[$pid]['status'] ,1)) {
     ...
     * @return mixed
     */
    public static function _getPostStick($tid)
    {
        $sql = '
            SELECT p.*
            FROM %t s , %t p
        ...
        return DbUtils::getDzDbUtils(true)->queryAll($sql, array(
            'forum_poststick', 'forum_post', $tid
        ));
    }
}
