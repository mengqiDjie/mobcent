...
/////////////////////////////////////////////////////////////////////
    ...
    public static function _getFollowUsersByRange($uid, $page, $pageSize, $longitude, $latitude,  $sql, $radius) {
...
//////////////////////////////////////////////////////////////////////////
    ...
    // 获取用户粉丝的详细信息按距离排序
    public static function _getFollowedUsersByRange($uid, $page, $pageSize, $longitude, $latitude,  $sql, $radius) {
        $range = self::_getRange($longitude, $latitude, $radius);
...
///////////////////////////////////////////////////////////////
    ...
    public static function _getRecommendUsersSetByDefault($page, $pageSize) {
        return DbUtils::getDzDbUtils(true)->queryColumn('
            ...
            LIMIT %d, %d
            ',
            array('home_specialuser', 0, $pageSize*($page-1), $pageSize)
        );
    }
...
//////////////////////////////////////////////////////////////////
    ...
    //查出uid下的好友
    public static function _getFrienduid($uid,$page, $pageSize) {
        $ids = DbUtils::getDzDbUtils(true)->queryAll('
            SELECT DISTINCT hf.uid FROM %t hf, %t st WHERE fuid = %d
            AND hf.uid = st.uid
            ORDER BY st.lastvisit
            LIMIT %d,%d
        ',array(
            'home_friend',
            'common_member_status',
    ...
    //查出关注好友
    public static function _getFollowuid($uid,$page, $pageSize) {
        $ids = DbUtils::getDzDbUtils(true)->queryAll('
            SELECT DISTINCT hf.followuid FROM %t hf, %t st WHERE hf.uid = %d
            AND hf.uid = st.uid
            ORDER BY st.lastvisit
            LIMIT %d,%d
        ',array(
    ...
    //查出粉丝
    public static function _getFolloweduid($uid,$page, $pageSize) {
        $ids = DbUtils::getDzDbUtils(true)->queryAll('
            SELECT DISTINCT hf.uid FROM %t hf, %t st WHERE hf.followuid = %d
            AND hf.uid = st.uid
            ORDER BY st.lastvisit
            LIMIT %d,%d
        ',array(
    ...
    // 用户好友列表按距离排序
    public static function _getPostFuidListByRange($uid, $page, $pageSize, $longitude, $latitude, $sql,$radius) {
        $range = self::_getRange($longitude, $latitude, $radius);
...
///////////////////////////////////////////////////////////// 
    ...
    //获取用户男女的id
    public static function _getRecommendGenderId($gender) {
       $ids = DbUtils::getDzDbUtils(true)->queryAll('
            SELECT uid
            FROM %t
            WHERE gender = %d
            ',
            array('common_member_profile', $gender)
        );
        if(empty($ids)) {
            return "";
        }
