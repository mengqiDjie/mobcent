...
class TopicUtils {
   ...
   public static function getActivityInfo($tid) {
        ...
        if (!empty($activityInfo)) {
            ...
            $activity['newsummary'] = self::_getActivitySummaryByJson($activityInfo);
    ...
    private static function _getActivitySummaryByJson($activity)
    {
        $startTime = $activity['starttimefrom'] . ($activity['starttimeto'] ? WebUtils::t(' 至 ').$activity['starttimeto'].WebUtils::t(' 商定') : '');
        $gender = WebUtils::t($activity['gender'] == 1 ? '男' : ($activity['gender'] == 2 ? '女' : '不限'));
        $allApplyNums = $activity['mobcent']['allApplyNums'];
        $res = array(
            array('type' => 'type', 'title' => WebUtils::t('活动类型'), 'value' => $activity['class']),
            array('type' => 'begintime', 'title' => WebUtils::t('开始时间'), 'value' => $startTime),
            array('type' => 'place', 'title' => WebUtils::t('活动地点'), 'value' => $activity['place']),
            array('type' => 'gender', 'title' => WebUtils::t('性别'), 'value' => $gender),
            array('type' => 'cost', 'title' => WebUtils::t('每人花销'), 'value' => $activity['cost']),
            array('type' => 'number', 'title' => WebUtils::t('已报名人数'), 'value' => $allApplyNums),
            array('type' => 'surplus', 'title' => WebUtils::t('剩余报名人数'), 'value' => $activity['number']),
            array('type' => 'endtime', 'title' => WebUtils::t('报名截止'), 'value' => $activity['expiration']),
        ...
        if (!empty($userFields)) { 
            foreach($userFields as $field) {
                ...
                if ($option['available']) {
                    ...
                    }elseif ($option['formtype'] == 'radio'){
                        $applyInfo['options'][] = self::_makeInputRadio($option,$value);
                    } else {
                        $applyInfo['options'][] = self::_makeInputElement(
                            $option['formtype'], $option['fieldid'],
    ...
    private static function _makeInputRadio($data,$value) {
        $ele = explode("\n",$data['choices']);
        foreach($ele as $v){
            $tmp['name'] = $data['fieldid'];
            $tmp['type'] = $data['formtype'];
            $tmp['value'] = $v;
            $tmp['label'] = $v;
            $elearr[] = $tmp;
        }
        return array('type' => $data['formtype'], 'name' => $data['fieldid'], 'value' => $value,
            'label' => $data['title'], 'attributes' => array('required' => 1), 'elements' => $elearr,
        );
    }
}