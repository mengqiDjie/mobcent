...
class PortalUtils {
        ...
        if (!empty($article)) {
            ...
            if ($article['pic']) {
                ...
                if ($allowImage) {
                    ...
                    if ($getImageList) {
                        $tempImage = WebUtils::getHttpFileName($summary['image']);
     ...
     * @return mixed
     */
    public static function getNewsContentHtml($aid, $page=1) {
        global $_G;
        ...
        $content['content'] = PortalUtils::preHtmlArticleContent($content['content']);
     ...
     * @return mixed
     */
    public static function preHtmlArticleContent($content) {
        ...
        $content = preg_replace_callback(
            ...
            create_function('$matches', '
                ...
                switch ($matches[1]) {
                    case "=mp3":
                        $string = "<embed src={$matches[2]}>";
                        break;
                    default:
                        $videoUrl = $matches[2];
                        $string = "<embed width=\'500\' height=\'375\' src={$videoUrl}>";
                        break;
                }
                return $string;
            '),
        ...
        $content = preg_replace_callback('/<a href="(.*?)".*?>(.*?)<\/a>/is',
            create_function('$matches', '
                ...
                if (stripos($matches[2], "<img") !== false) {
                    ...
                    $string = sprintf("<a href=%s>%s</a>", $matches[1], $matches[2]);
                }
                return $string;
            '),
            $content
        ...
        $content = preg_replace_callback('/<img.*?src="(.*?)".*?>/is',
            create_function('$matches', '
                ...
                if (strpos($matches[1], "static/image/smiley") !== false) {
                    $string = "<img src={$matches[1]}>";
                } else {
                    $string = sprintf("<img src=%s>", $matches[1]);
                }
                return $string;
            '),
            $content
        );
        $content = self::html_decode($content);
        return $content;
    }
    ...
    //返回是否收藏了文章
    public static function isFavoriteArticle($uid, $aid)
    {
        return DzPortalArticle::getArticleCountByFav($uid, $aid);
    }

    //替换HTML实体
    public static function html_decode($document)
    {
        $search = array (
...
function parseforumattach(&$post, $aids) {
    ...
    if(($aids = array_unique($aids))) {
        ...
        foreach(C::t('forum_attachment_n')->fetch_all_by_id('tid:'.$post['tid'], 'aid', $aids) as $attach) {
            ...
            if($attach['remote']){
                if(!empty(Yii::app()->params['qiniu']) && Qiniuup::haveImg($attach['url'])){
                    $attach['url'] = $_G['setting']['ftp']['attachurl'];
                }else{
                    $attach['url'] = $_G['setting']['ftp']['attachurl'].'forum/';
                }
            }else{
                $attach['url'] = $_G['setting']['attachurl'].'forum/';
            }
