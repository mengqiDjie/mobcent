...
class DB extends discuz_database {
    ...
    public static function fetch_all($sql, $arg = array(), $keyfield = '', $silent=false) {
        ...
        if(self::$driver=='db_driver_mysqli'&&function_exists('mysqli_fetch_all')){
            $row = mysqli_fetch_all($query,MYSQLI_ASSOC);
            foreach ($row as $key=>$value){
                if ($keyfield && isset($value[$keyfield])) {
                    $data[$row[$keyfield]] = $value;
                } else {
                    $data[] = $value;
                }
            }
...
class MobcentDiscuzApp extends discuz_application {
    ...
    public function _init_longuage()   
    {   
        $language='zh_cn';
        if(isset($_REQUEST['lang'])&&$_REQUEST['lang']!="")   //通过lang参数识别语言   
        {   
            $language=strtolower(str_replace('-','_',$_REQUEST['lang']));   
            setcookie('lang',$_REQUEST['lang']);   
        }else if(isset($_COOKIE['lang'])&&$_COOKIE['lang']!="")   //通过$_COOKIE['lang']识别语言   
        {   
            $language=strtolower(str_replace('-','_',$_COOKIE['lang']));   
        }else if(isset($_SERVER['HTTP_ACCEPT_LANGUAGE'])){   //通过系统或浏览器识别语言   
        ...
        */
        ...
        if(isset($language_dict[$language])){
