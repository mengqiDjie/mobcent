...
class CommentListAction extends MobcentAction {
    ...
    protected function getCommentList($data) {
        ...
        $list = array();
        $comments = DzPortalComment::getComments($data['id'], $data['idType'], $data['page'], $data['pageSize'], $data);
        foreach ($comments as $comment) {
            ...
            $tmpComment['userTitle'] = UserUtils::getUserTitle($comment['uid']);
            $tmpComment['userColor'] = UserUtils::getUserColor($comment['uid']);
            $tmpComment['level']     = $this->_getUserLevel($comment['uid']);
            $tmpComment['gender']    = UserUtils::getUserGender($comment['uid']);
            ...
            if($data['bbcode']) {
                $tmpComment['html_reply_content'] = (PortalUtils::preHtmlArticleContent($comment['message']));
            }
            $list[] = $tmpComment;
        }
        $res['list'] = $list;
    ...
    protected function transCommentMessage($message) {
        ...
        foreach ($message as $line) {
            $line = PortalUtils::html_decode($line);
            $matches = array();
