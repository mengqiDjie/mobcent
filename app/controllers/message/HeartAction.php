...
class HeartAction extends MobcentAction {
    ...
    private function _getPmInfos($uid) {
        $pmInfos = array();
        if(WebUtils::getDzPluginAppbymeAppConfig('gotye')){
           return $pmInfos;
        }
        loaducenter();
    ...
    //获得系统消息ByNxx
    private function _getSystemInfo($uid) {
        $data = DbUtils::getDzDbUtils(true)->queryAll('
            SELECT *
            FROM %t
            WHERE uid=%d AND (type=%s OR (type=%s)) AND new=%d
            ORDER BY dateline DESC
            ', array('home_notification', $uid, 'friend', 'system', 1)
        );
        $info = array(
            'count' => count($data),
            'time' => !empty($data) ? $data[0]['dateline'] . '000' : "0",
